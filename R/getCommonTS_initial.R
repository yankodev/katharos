...
#' securities/indices in the price list pxList.
...
getCommonTS_DLY = function (pxList) {
  # source('//VKBADS.DE/ORG/VKB/TECTA/TEC103/ALLE/12_Punchev/Code/katharos/01Functions/01TimeSeries/getCommonTS.R')
  # add diagnositcs - determine frequency of each time series
  library(data.table)

  minDate = max(as.Date(unlist(lapply(pxList, function(x) min(as.Date(x$DATE)))), origin = "1970-01-01"))     # first possible common date
  maxDate = min(as.Date(unlist(lapply(pxList, function(x) max(as.Date(x$DATE)))), origin = "1970-01-01"))     # last possible common date

  dtVec_temp = data.table()
  dtVec_temp = lapply(pxList, function(x) dtVec_temp = rbind(data.frame(DATE = x$DATE), dtVec_temp))
  ...
  dtVec_temp = dtVec_temp[count >= length(pxList) / 3, ]   # only choose date points, for which at leat 1/3 of the TS have an observation
  dtVec = data.table(DATE = as.Date(dtVec_temp$DATE, origin = "1970-01-01"))
  setkey(dtVec, DATE)
  dtVec = dtVec[order(DATE)]
  dtVec = dtVec[DATE >= minDate & DATE <= maxDate]

  dtCommonTS = dtVec

  for (j in 1:(length(pxList))){
    dtRawPriceTS = data.table(pxList[[j]][, c('DATE','INDEX')])
    ...
    dtCommonTS[, eval(names(pxList)[j])] = ifelse(   # interpolate, if necessary
  ...
  for (j in 2:ncol(dtCommonTS)){  # assume, that if min(TS) < 0.05 we're handling interest rates
    ...
    if (min(dtCommonTS[, get(colnames(dtCommonTS)[j])]) < 0.05 ||  !is.null(pxList[[j-1]]$OR)){
      # dtCommonTS[, j] = dtCommonTS[, get(colnames(dtCommonTS)[j])] - dtCommonTS[1, get(colnames(dtCommonTS)[j])]  # first difference based on unajusted TS
...
getCommonTS_WKL = function (pxList)
{
  library(data.table)
  ...
  for (j in 2:ncol(dtCommonTS)){  # assume, that if min(TS) < 0.05 we're handling interest rates
    if (min(dtCommonTS[, get(colnames(dtCommonTS)[j])]) < 0.05 ){
      dtCommonTS[, j] = dtCommonTS[, get(colnames(dtCommonTS)[j])] - dtCommonTS[1, get(colnames(dtCommonTS)[j])]  # first difference based on unajusted TS
      dtCommonFD[, j] = diff(dtCommonTS[, get(colnames(dtCommonTS)[j])])
    } else {
      dtCommonTS[, j] = dtCommonTS[, get(colnames(dtCommonTS)[j])] / dtCommonTS[1, get(colnames(dtCommonTS)[j])]  # first difference base on LOG(TS)
...
getCommonTS_MTL = function (pxList)
{
  library(data.table)
  ...
  dtVec = seq.Date(as.Date(paste(year(minDate), month(minDate), 1, sep ="-")), length = ceiling(as.numeric(maxDate - minDate) / 30) + 2, by="1 month")-1
...
getCommonTS_WKL_InterpolFlag = function (pxList, boolInterpolate)
{
  library(data.table)
  ...
  for (j in 1:(length(pxList))){
    ...
    if(boolInterpolate){
      ...
      dtCommonTS[, eval(names(pxList)[j])] = dtMergeTS$INDEX_BackWard   # do NOT interpolate under any circumstances,
                                                                        # take last avialbe value
    }
...
getCommonTS_MTL_InterpolFlag = function (pxList, boolInterpolate)
{
  library(data.table)
